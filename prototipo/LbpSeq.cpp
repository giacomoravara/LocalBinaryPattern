...
LbpSeq::LbpSeq(const std::vector<byte>& pixels, unsigned width, unsigned height) :
...
byte LbpSeq::pixelAt(unsigned row, unsigned col)
{
	return pixels[row * width + col];
}

float *LbpSeq::getHistogram(float *histograms, unsigned row, unsigned col)
{
...
unsigned LbpSeq::compareWithNeighborhood(unsigned row, unsigned col)
{
...
std::vector<float> LbpSeq::calculateNormalizedLBPs(float radius, unsigned samples, unsigned blockEdge)
{
	return calculateNormalizedLBPs(radius, samples, blockEdge, nullptr, nullptr, nullptr);
}

std::vector<float> LbpSeq::calculateNormalizedLBPs(float radius, unsigned samples, unsigned blockEdge, std::vector<byte> *outputImage, unsigned *outputWidth, unsigned *outputHeight)
{
