...
LbpCuda::LbpCuda(const std::vector<byte>& pixels, unsigned width, unsigned height) :
...
std::vector<float> LbpCuda::calculateNormalizedLBPs(float radius, unsigned samples, unsigned blockEdge)
{
...
void LbpCuda::calcHistGridAndBlockSize(dim3& grid, dim3& block, unsigned& remainder)
{
...
void LbpCuda::calcLbpGridAndBlockSize(dim3& grid, dim3& block)
{
...
struct cudaDeviceProp LbpCuda::getDeviceProps() {
	int device;
